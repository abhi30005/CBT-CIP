...
char* get_highest_score(stud* stud) {
    int max_index = 0;
    for (int i = 1; i < stud->num_subjects; i++) {
        if (stud->scores[i] > stud->scores[max_index]) {
            max_index = i;
        }
    }
    return stud->subjects[max_index];
}

char* get_lowest_score(stud* stud) {
    int min_index = 0;
    for (int i = 1; i < stud->num_subjects; i++) {
    ...
    return stud->subjects[min_index];
}

void display_performance(stud* stud){
    char* highest_score = get_highest_score(stud);
    char* lowest_score = get_lowest_score(stud);
    printf("Student: %s %s\n",strupr(stud->name),strupr(stud->n1));
    printf("\nAverage Score: %.2f\n",calculate_average(stud));
    printf("Highest Score --> %s : %.2f\n", highest_score, stud->scores[stud->num_subjects - 1]);
    printf("Lowest Score --> %s : %.2f\n", lowest_score, stud->scores[0]);
    printf("\n");
}
